...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState* state, int handPos, int* bonus)
{
	...
	case baron:
		baron_play(state, choice1, currentPlayer);
		return 0;
...
int ambassador_play(int choice1, int choice2, int handPos, struct gameState* state, int currentPlayer)
{
	int j = 0;	//check if a player has enough cards to discard
	int i;
	...
	for (i = 0; i < state->handCount[currentPlayer]; i++)
	{
		if (i == state->hand[currentPlayer][choice1])	//Don't need the extra because choice1 est. that choice cant be amb. card.
		{
			j++;
		}
	}
...
int baron_play(struct gameState* state, int choice1, int currentPlayer)
{
	...
	{	//Boolean true or going to discard an estate
		...
		while (card_not_discarded)
		{
			...
			else if (p > state->handCount[currentPlayer])
			{
				...
				{
					estateGainCheck(state, currentPlayer);	//Gain an estate, decrements estate supply, checks is game over
				}
				card_not_discarded = 0;	//Exit the loop
			}
			else
...
{
	gainCard(estate, state, 0, currentPlayer);	//Gain estate, decrements estate supply	
...
int mine_play(struct gameState* state, int choice1, int choice2, int currentPlayer, int handPos)
{
	...
	if (state->hand[currentPlayer][choice1] < copper || state->hand[currentPlayer][choice1] >= gold)	//must be treasure
	{
		return -1;
	}

	if (choice2 > treasure_map || choice2 < curse)	//must be valid card range
	{
		return -1;
	}
	...
	gainCard(choice2, state, 0, currentPlayer);	//Gain card to hand

	//discard card from hand
	discardCard(handPos, currentPlayer, state, 0);	//discard index of card in player's hand
...
int tribute_play(struct gameState* state, int tributeRevealedCards[], int currentPlayer, int nextPlayer)
{
	...
	else
	{
		if (state->deckCount[nextPlayer] == 0)
		{
			...
			{
				nPlayerDiscardToDeck(state, nextPlayer, i);
				//move to deck function
